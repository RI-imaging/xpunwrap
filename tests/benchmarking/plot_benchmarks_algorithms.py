...
def load_results() -> List[Dict[str, object]]:
    if not RESULTS_JSON.exists():
        raise FileNotFoundError(
            "benchmark_results.json not found. Run pytest on tests/test_benchmark_algorithms.py first."
        )
...
def _plot(results, use_transfers: bool, output_path: Path):
    ...
    backends = sorted({r["backend"] for r in results}, key=lambda b: (b != "numpy", b))
    ...
    for ax, (title, algos) in zip(axes, _group_defs()):
        ...
        for idx, backend in enumerate(backends):
            backend_results = {r["algorithm"]: r for r in results if r["backend"] == backend}
            if use_transfers:
                means = [
                ...
                stds = [backend_results[a].get("std_proc_ms", 0.0) for a in target_algos]
        ...
        ax.set_title(title + (" (transfer only)" if use_transfers else ""))
        ax.set_xticks(x)
        ax.set_xticklabels([a.replace("algo_", "") for a in target_algos], rotation=15, ha="right")
...
def plot(results: List[Dict[str, object]]) -> None:
    ...
    # Also print transfer times so they are visible even though not plotted.
    ...
    for backend in backends:
        ...
        for algo in algos_available:
            ...
            print(
                f"  {backend:5s} | {algo:22s} | H2D {h2d:7.2f}+/-{h2d_std:5.2f} | "
